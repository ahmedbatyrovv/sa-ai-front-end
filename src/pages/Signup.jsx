// src/pages/Signup.jsx (similar fallbacks added)
...
function Signup({ onSignup, onSwitchToLogin, toast, setToast }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (password.length < 6) {
      setError(t('password-too-short') || 'Password must be at least 6 characters');
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      if (err.response && err.response.status === 400) {
        const errorMsg = err.response.data.errors ? err.response.data.errors[0]?.msg || t('validation-error') || 'Validation error' : err.response.data.message || t('registration-failed') || 'Registration failed';
        setError(errorMsg);
      } else if (err.response && err.response.status === 500) {
        setError(t('server-error') || 'Server error');
      } else {
      ...
      <div className="auth-modal">
        ...
        <form onSubmit={handleSubmit} className="auth-form">
          <div className="form-group">
            ...
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder={t('full-name') || 'Full name'} disabled={loading} />
          </div>
          <div className="form-group">
