...
function Login({ onLogin, onSwitchToSignup, toast, setToast }) {
  ...
  const handleSocialLogin = async (provider) => {
    ...
    try {
      ...
      setLocalToast({ message: t('logged-in-with-provider', { provider: capitalizeProvider(provider) }), type: 'success' });
      setTimeout(() => {
        setLocalToast(null);
      ...
      if (err.response && err.response.status === 400) {
        const errorMsg = err.response.data.message || t('failed-with-provider', { action: t('log-in').toLowerCase(), provider: capitalizeProvider(provider) });
        setError(errorMsg);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      localStorage.setItem('token', response.data.token);
      localStorage.setItem('currentUser', JSON.stringify(response.data.user));
      setLocalToast({ message: t('logged-in-success'), type: 'success' });
      setTimeout(() => {
        setLocalToast(null);
        if (onLogin) onLogin(response.data.user);
      }, 3000);
      ...
      if (err.response && err.response.status === 400) {
        const errorMsg = err.response.data.message || t('invalid-credentials');
        setError(errorMsg);
      } else if (err.response && err.response.status === 500) {
        setError(t('server-error'));
      } else {
    ...
    <div className="auth-container">
      <div className="auth-modal">
        ...
        <div className="auth-footer">
          <p>{t('dont-have-account')} <button onClick={onSwitchToSignup} className="auth-link">{t('sign-up')}</button></p>
        </div>
      </div>
